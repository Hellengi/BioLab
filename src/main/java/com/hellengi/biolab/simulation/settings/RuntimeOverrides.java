...
public class RuntimeOverrides {
    ...
    private Double globalLightCycleElapsedTicks;
    ...
    public double getGlobalLight() {
        ...
        double phase = (getGlobalLightCycleElapsedTicks() % periodTicks) / periodTicks;
        double wave = (Math.sin(phase * Math.PI * 2.0 - Math.PI / 2.0) + 1.0) / 2.0;

        return min + (max - min) * wave;
    }

    public void advanceGlobalLightCycle(double tickScale) {
        if (!isGlobalLightCycleEnabled()) {
            return;
        }

        this.globalLightCycleElapsedTicks = getGlobalLightCycleElapsedTicks() + tickScale;
    }

    public double getGlobalLightCycleElapsedTicks() {
        return globalLightCycleElapsedTicks != null ? globalLightCycleElapsedTicks : 0.0;
    }
    ...
    public void reset() {
        ...
        this.globalLightCycleElapsedTicks = null;
    ...
    public void applySnapshot(SimulationSettingsDto dto) {
        apply(dto);
        this.globalLightCycleElapsedTicks = Math.max(0.0, dto.globalLightCycleElapsedTicks());
    }
